...
{	
	...
	sprintf(pathImages,"D:\\EyeBDA\\Images-Validation\\");
	sprintf(pathOutput1,"D:\\EyeBDA\\Experiments%s\\Cascade-LAC\\Node%s\\Validation\\",ExperimentStr,NodeStr);
	sprintf(pathOutput2,"ResultsOfScanningLAC\\");
	
	nImages = NUM_IMAGES;

	FILE *FP_EyeCoord;
	sprintf(StrTemp,"D:\\EyeBDA\\Val_Eye_Coord.dat");
	...
	do {
		...
		LoadingGrayBMP(StrTemp,ImgMat);		// 원본사이즈의 bmp영상의 pixel intensities가 변수 ImgMat에 저장
		...
		while (Flag==0)
		{
			...
			for (i=0;i<ImgHeightLevel-WIN_SIZE+1;i++)
			{
				for (j=0;j<ImgWidthLevel-WIN_SIZE+1;j++)
				{
					...
					//printf("ImgWidth:%d, ImgHeight:%d, ImgWidthLevel:%d, ImgHeightLevel:%d \n",ImgWidth,ImgHeight,ImgWidthLevel,ImgHeightLevel);
					...
					if (PatchScore2 <= 0.125)
					{
						nPos = nPos + 1;
						TrueClass = 1;
						//fprintf(FP_TP,"%d, %d, %d, %d, %d, %d, %f\n",ImgWidthLevel,ImgHeightLevel,j+1,i+1,(j+WIN_SIZE),(i+WIN_SIZE),PatchScore1);
					}
					else if (PatchScore2 >= 0.5)
					{
						nNeg = nNeg + 1;
						TrueClass = 0;
						//if (nNeg%PRINT_UNIT_FALPOS==1)
						//{
						//	fprintf(FP_FP,"%d, %d, %d, %d, %d, %d, %f\n",ImgWidthLevel,ImgHeightLevel,j+1,i+1,(j+WIN_SIZE),(i+WIN_SIZE),PatchScore);
						//}
					}
					else TrueClass = -1;

					NodeIndex = 0;		// NodeIndex는 0부터 시작!!
					EstClass = 1;
					count = 0;
