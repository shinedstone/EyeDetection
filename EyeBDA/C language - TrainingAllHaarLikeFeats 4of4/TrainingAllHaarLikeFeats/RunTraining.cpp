...
void main()
{
	...
	sprintf(StrTemp,"D:\\EyeBDA\\Experiments%s\\HaarFeatInfo18x18_4of4.dat",ExperimentsIndex);
	...
	TimeStart = clock();					
	sprintf(StrTemp,"D:\\EyeBDA\\Experiments%s\\Cascade-BDA\\Node%s\\Training\\TrainSamplesIN_Node%s.dat",ExperimentsIndex,NodeIndex,NodeIndex);
	...
	for (k=0;k<NUM_HAARFEATS;k++) {
		...
		int EstClass = 0;
		for (i=0;i<(nSampPos+nSampNeg);i++) {
			if (FeatValues[i]*Polarity >= Threshold*Polarity) EstClass=1;
			else EstClass=0;
