...
void main()
{
	...
	sprintf(StrTemp,"D:\\EyeDetection\\Experiments%s\\HaarFeatInfo18x18_1of4.dat",ExperimentsIndex);
	...
	TimeStart = clock();					
	sprintf(StrTemp,"D:\\EyeDetection\\Experiments%s\\Cascade-LAC\\Node%s\\Training\\TrainSamplesIN_Node%s.dat",ExperimentsIndex,NodeIndex,NodeIndex);
	...
	for (k=0;k<NUM_HAARFEATS;k++) 
	{
		...
		TimeEnd = clock();
		printf("HaarFeats: %d---------------\nThreshold: %lf, Polarity: %d\n(e-Time: %.3f, RecogRates: %.2f)\n",k+1,Threshold,Polarity,(TimeEnd-TimeStart)/CLOCKS_PER_SEC,RecogRates);
	}
	fclose(FP_ThetaP);
	fclose(FP_VMat);
