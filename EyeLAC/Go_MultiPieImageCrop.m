...
RstImg_folder = 'TestImages3';
imgout_ftype = 'bmp';

fid = fopen('eyeMultipie_eye_mouth_result.txt','r');
fid_out = fopen('Total_Eye_Coord3.dat','w');

folder = 'Z:\FaceDB\Multi-Pie\data\session01\multiview';
imgin_ftype = 'png';
...
for i = 1 : 199
    ...
    folder_name1 = cur_string(1:blank_pos-1);
    for j=blank_pos+1:length(cur_string)
        if cur_string(j) == '_',
            blank_pos = j;
            break;
        end
    end
    folder_name2 = cur_string(blank_pos-2:blank_pos-1);
    for j=blank_pos+1:length(cur_string)
        if cur_string(j) == '_',
            blank_pos = j;
            break;
    ...
    folder_name3 = [cur_string(blank_pos-3:blank_pos-2) '_' cur_string(blank_pos-1)];
    for j=blank_pos+1:length(cur_string)
        if cur_string(j) == ' ',
            blank_pos = j;
            break;
    ...
    file_name = cur_string(1:blank_pos-5);
    ...
    img_file = sprintf('%s/%s/%s/%s/%s.%s', folder, folder_name1, folder_name2, folder_name3, file_name, imgin_ftype);
...
%     img_temp(reye_Y-1:reye_Y+1,reye_X-1:reye_X+1)=255;
    reye_file = sprintf('%s/%s_reye.%s', RstImg_folder, file_name, imgout_ftype);
...
%     img_temp(leye_Y-1:leye_Y+1,leye_X-1:leye_X+1)=255;
    leye_file = sprintf('%s/%s_leye.%s', RstImg_folder, file_name, imgout_ftype);
