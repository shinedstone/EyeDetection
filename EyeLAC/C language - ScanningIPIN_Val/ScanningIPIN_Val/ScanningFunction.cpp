...
unsigned char NodeClassificationINBDA(CSL_StrongClassifierBDA* NodeClassifier, unsigned char NodeIndex, long long* ImgIntgMat, unsigned int WidthImg, CSL_ImgPatch* ImgPat)
{
	int i=0, k=0;
	unsigned char Flag, type;
	int size_y, size_x, start_y, start_x, polarity;
	...
	/*
	printf("HaarFeatVec: ");
	for (i=0;i<NodeClassifier[NodeIndex].nHaarFeat;i++) printf("%d ",HaarFeatVec[i]);
	printf("\n");

	printf("MeanTrainPos: ");
	for (i=0;i<NodeClassifier[NodeIndex].nHaarFeat;i++) printf("%f ",NodeClassifier[NodeIndex].MeanTrainPos[i]);
	printf("\n");
		
	printf("Weight BDAGM:\n");
	for (i=0;i<NodeClassifier[NodeIndex].nHaarFeat;i++) printf("%f ",(NodeClassifier[NodeIndex].Weights)[NodeClassifier[NodeIndex].nBDAFeat*i+0]);
	printf("\n");
	for (i=0;i<NodeClassifier[NodeIndex].nHaarFeat;i++) printf("%f ",(NodeClassifier[NodeIndex].Weights)[NodeClassifier[NodeIndex].nBDAFeat*i+1]);
	printf("\n");

	printf("OutFeatVec:");*/
	OutValueTotal = 0;
	for(k=0;k<NodeClassifier[NodeIndex].nBDAFeat;k++) {
		OutValue = 0;		//OutValue is temporary variable.
		for (i=0;i<NodeClassifier[NodeIndex].nHaarFeat;i++) {
	...
	//printf("OutValueTotal=%f, EnTheta=%f\n",OutValueTotal,NodeClassifier[NodeIndex].EnTheta);
