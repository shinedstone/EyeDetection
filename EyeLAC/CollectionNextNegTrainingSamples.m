...
ScaleFactor = 0.8;
nNeg = 2800;
MaxScoreForNeg = 0.125;
MinScoreNeg = 10000;

FlagTrVal = 1;
if FlagTrVal == 1
    FolderImages = 'Images-Training';
    FolderPath = ['Experiments' ExperimentsIndex '\Cascade-LAC\Node0' num2str(str2double(NodeIndex)-1) '\Training'];
    FolderPathOut = ['Experiments' ExperimentsIndex '\Cascade-LAC\Node' NodeIndex '\Training'];
elseif FlagTrVal == 2
    FolderImages = 'Images-Validation';
    FolderPath = ['Experiments' ExperimentsIndex '\Cascade-LAC\Node0' num2str(str2double(NodeIndex)-1) '\Validation'];
...
for i=1:nFiles
    ...
    FPs = load([FolderPath '\ResultsOfScanningLAC\LAC_FP_0' num2str(str2double(NodeIndex)-1) '_' FileName '.dat']);
    nFPs = size(FPs,1);
    if nFPs > 0
