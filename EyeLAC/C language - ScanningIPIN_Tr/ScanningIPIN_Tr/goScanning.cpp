...
//#define		NUM_IMAGES					2800
//#define		NUM_IMAGES					4660
//#define		NUM_IMAGES					2400
#define		NUM_IMAGES					398
...
{	
	...
	CSL_StrongClassifier NodeClassifier[NumOfNode];
	CSL_ImgPatch Patch;

	LoadingNodeClassifierIN(NodeClassifier,NumOfNode,1);

	printf("Scanning Test Images\n");
	//sprintf(pathImages,"D:\\EyeDetection\\Images-Training\\");
	//sprintf(pathImages,"D:\\EyeDetection\\TestImages\\");
	//sprintf(pathImages,"D:\\EyeDetection\\TestImages2\\");
	sprintf(pathImages,"D:\\EyeDetection\\TestImages3\\");
	sprintf(pathOutput1,"D:\\EyeDetection\\Experiments%s\\Cascade-LAC\\Node%s\\Training\\",ExperimentStr,NodeStr);
	sprintf(pathOutput2,"ResultsOfScanningLAC\\");
	
	nImages = NUM_IMAGES;
	
	FILE *FP_EyeCoord;
	//sprintf(StrTemp,"D:\\EyeDetection\\Tr_Eye_Coord.dat");
	//sprintf(StrTemp,"D:\\EyeDetection\\Total_Eye_Coord.dat");
	//sprintf(StrTemp,"D:\\EyeDetection\\Total_Eye_Coord2.dat");
	sprintf(StrTemp,"D:\\EyeDetection\\Total_Eye_Coord3.dat");
		...
		while (Flag==0)
		{
			...
			for (i=0;i<ImgHeightLevel-WIN_SIZE+1;i++)
			{
				for (j=0;j<ImgWidthLevel-WIN_SIZE+1;j++)
				{
					...
					if (PatchScore2 <= 0.125)
					{
						nPos = nPos + 1;
						TrueClass = 1;
					}
					else if (PatchScore2 >= 0.5)
					{
						nNeg = nNeg + 1;
						TrueClass = 0;
					}
					...
					{
						EstClass = NodeClassificationIN(NodeClassifier,NodeIndex,ImgMatIntg,ImgWidthLevel,&Patch,&count);
